...
CACHE_DIR = '/Users/junny/.openclaw/cache'
TIMELINE_CACHE_FILE = os.path.join(CACHE_DIR, 'wh_stock_timeline_cache.pkl')

def load_timeline_cache():
    if os.path.exists(TIMELINE_CACHE_FILE):
        try:
            with open(TIMELINE_CACHE_FILE, 'rb') as f:
                return pickle.load(f)
        except Exception:
            return {}
    return {}

def save_timeline_cache(cache):
    try:
        os.makedirs(CACHE_DIR, exist_ok=True)
        with open(TIMELINE_CACHE_FILE, 'wb') as f:
            pickle.dump(cache, f)
    except Exception as e:
        logger.warning("保存库存时间序列缓存失败: %s", e)

def _extract_wh_timestamp_str(fpath):
...
def build_inventory_timeline(wh_files, max_snapshots=18):
    ...
    recent_files = sorted_files[-max_snapshots:] if len(sorted_files) > max_snapshots else sorted_files
    
    snapshots = []
    cache_dirty = False
    
    for fpath in recent_files:
        ts_str = _extract_wh_timestamp_str(fpath)
        try:
            dt_obj = datetime.strptime(ts_str[:14], '%Y%m%d%H%M%S')
        except Exception:
            dt_obj = datetime.fromtimestamp(os.path.getmtime(fpath))
            
        mtime = os.path.getmtime(fpath)
        cache_key = (fpath, mtime)
        
        if cache_key in timeline_cache:
            skc_stock = timeline_cache[cache_key]
        else:
            ...
            try:
                ...
                wb.close()
                continue
            spec_idx = h.index('规格编码') if '规格编码' in h else -1
            stock_idx = h.index('库存数') if '库存数' in h else -1
            wh_code_idx = h.index('虚拟仓编码') if '虚拟仓编码' in h else -1
            wh_name_idx = h.index('虚拟仓名称') if '虚拟仓名称' in h else -1
            
            skc_stock = {}
            for r in rows:
                if wh_code_idx >= 0 and r[wh_code_idx] and (str(r[wh_code_idx]).startswith('TW201') or '退货' in str(r[wh_name_idx] or '')):
                    continue
                spec = str(r[spec_idx]).strip() if spec_idx >= 0 and r[spec_idx] else ''
                if len(spec) >= 10:
                    skc = spec[:10]
                    val = float(r[stock_idx]) if stock_idx >= 0 and r[stock_idx] is not None else 0.0
                    skc_stock[skc] = skc_stock.get(skc, 0.0) + val
            wb.close()
            timeline_cache[cache_key] = skc_stock
            cache_dirty = True
            
        snapshots.append((dt_obj, skc_stock))
        
    if cache_dirty:
        save_timeline_cache(timeline_cache)
        
    snapshots.sort(key=lambda x: x[0])
    return snapshots

def check_restock_inbound_status(snapshots, skc, shipped_val, total_qty_val, ship_date, plan_date, is_shipped_clean_raw, orig_status, today_date):
    """
    根据物理库存时间序列增减量（ΔStock）精准判定该批翻单是否真正准确入库：
    返回: (inbound_status, effective_date, status_desc, is_cleared)
    - inbound_status: 'ARRIVED' | 'IN_TRANSIT' | 'IN_PRODUCTION'
    - effective_date: 有效预计到仓/入库日期
...
def analyze():
    ...
    def is_older_than_two_weeks(source_name, base_date=today):
        """
        判断该翻单散表是否早于上上周一（14天以前的古老散表）
        """
        if not source_name:
            return False
        bname = os.path.basename(str(source_name))
        ...
        if m:
            try:
                ...
                if 1 <= mm <= 12 and 1 <= dd <= 31:
                    f_dt = datetime(base_date.year, mm, dd)
                    two_weeks_ago_monday = base_date - timedelta(days=base_date.weekday() + 7)
                    return f_dt < two_weeks_ago_monday
            except Exception:
                pass
        return False
        ...
        for sheet in wb1.sheetnames:
            ...
            for row_cells in ws1.iter_rows(min_row=header_num + 1):
                ...
                for rd in sub_rds:
                    ...
                    plan_idx = sg_indexes.get('plan_arrival')
                    act_idx = sg_indexes.get('actual_arrival')
                    status_idx = sg_indexes.get('status')
                    
                    shipped_val = 0.0
                    if shipped_idx is not None and len(rd) > shipped_idx and rd[shipped_idx] is not None:
                        try: shipped_val = float(rd[shipped_idx])
                        except: pass
                        
                    total_qty_val = 0.0
                    if total_qty_idx is not None and len(rd) > total_qty_idx and rd[total_qty_idx] is not None:
                        try: total_qty_val = float(rd[total_qty_idx])
                        except: pass
                        
                    plan_date = None
                    if plan_idx is not None and len(rd) > plan_idx:
                        plan_date = get_date_value(rd[plan_idx])
                        
                    ship_date = None
                    if act_idx is not None and len(rd) > act_idx:
                        ship_date = get_date_value(rd[act_idx])
                    ...
                    if cleared_idx is not None and len(rd) > cleared_idx:
                        ...
                        if any(x in cleared_val for x in ['清', '是', '已清', '完']):
                            is_raw_clean = True
                    if not is_raw_clean and row_yellow_clean:
                        is_raw_clean = True
                    if not is_raw_clean and shipped_val > 0 and total_qty_val > 0 and shipped_val >= total_qty_val:
                        is_raw_clean = True
                    ...
                    # 动态增减量判定（核心：只有监测到准确入库增量才算入库，否则视为生产完毕在途）
                    inbound_status, effective_date, status_desc, is_cleared = check_restock_inbound_status(
                        wh_snapshots_timeline, skc, shipped_val, total_qty_val, ship_date, plan_date,
                        is_raw_clean, orig_status, today.date()
                    )
                    ...
                    all_restock[key]['inbound_status'] = inbound_status
                    all_restock[key]['ship_date'] = ship_date
                    all_restock[key]['plan_date'] = plan_date
    ...
    if nba_file and os.path.exists(nba_file):
        ...
        for sheet in wb2.sheetnames:
            ...
            for row_cells in ws2.iter_rows(min_row=header_num + 1):
                ...
                plan_idx = nba_indexes.get('plan_arrival')
                act_idx = nba_indexes.get('actual_arrival')
                status_idx = nba_indexes.get('status')
                ...
                if cleared_idx is not None and len(rd) > cleared_idx:
                    ...
                    if any(x in cleared_val for x in ['清', '是', '已清', '完']):
                        is_raw_clean = True
                if not is_raw_clean and row_yellow_clean_nba:
                    is_raw_clean = True
                if not is_raw_clean and shipped_val > 0 and total_qty_nba > 0 and shipped_val >= total_qty_nba:
                    is_raw_clean = True
                ...
                # 动态增减量判定
                inbound_status, effective_date, status_desc, is_cleared = check_restock_inbound_status(
                    wh_snapshots_timeline, skc, shipped_val, total_qty_nba, ship_date, plan_date,
                    is_raw_clean, orig_status_nba, today.date()
                )
    ...
    # ── 核心判定：检测生产部大货总表本周是否已经更新 ──────────────────────────────
    master_files_check = [f for f in (list(sg_files) + list(nba_files)) if f and os.path.exists(f)]
    latest_master_mtime = None
    for mf in master_files_check:
        mt = datetime.fromtimestamp(os.path.getmtime(mf))
        if latest_master_mtime is None or mt > latest_master_mtime:
            latest_master_mtime = mt
            
    monday_start = today - timedelta(days=today.weekday())
    monday_start = monday_start.replace(hour=0, minute=0, second=0, microsecond=0)
    is_master_updated = (latest_master_mtime is not None and latest_master_mtime >= monday_start)
    
    logger.info("🎯 生产部大货总表更新状态判定: %s (文件最新修改时间: %s, 本周一基准点: %s)",
                "✅ 本周已更新" if is_master_updated else "❌ 本周未更新(停留在上周或更早)",
                latest_master_mtime.strftime('%Y-%m-%d %H:%M:%S') if latest_master_mtime else "无",
                monday_start.strftime('%Y-%m-%d'))
    ...
    if prev_wh_file:
        for skc, cur_total in wh_neg_total.items():
            ...
            if not prev_has_stock and cur_has_stock:
                ...
                # ── 维度一：库存变动与翻单表数量匹配联动提示（到货达成状态） ──────────────────
                order_kind_txt = '首单' if is_first_order else '翻单'
                if target_order_qty >= 10:
                    pct_str = f"{int(match_ratio * 100)}%"
                    if match_ratio >= 0.60:
                        qty_match_tag = f"🟢 {order_kind_txt}大货到齐 ({pct_str})"
                        qty_match_desc = f"🟢 {order_kind_txt}大货到齐({order_kind_txt}{target_order_qty}件/实到{int(inbound_delta)}件, 达成率{pct_str})"
                        match_level = 1
                    elif match_ratio >= 0.20 and inbound_delta >= 15.0:
                        qty_match_tag = f"🟡 {order_kind_txt}分批先到 ({pct_str})"
                        qty_match_desc = f"🟡 {order_kind_txt}分批先到({order_kind_txt}{target_order_qty}件/先到{int(inbound_delta)}件, 达成率{pct_str})"
                        match_level = 2
                    else:
                        qty_match_tag = f"⚪ {order_kind_txt}零星先到 ({pct_str})"
                        qty_match_desc = f"⚪ {order_kind_txt}零星先到({order_kind_txt}{target_order_qty}件/先到{int(inbound_delta)}件, 达成率{pct_str})"
                        match_level = 3
                else:
                    if inbound_delta >= 30.0:
                        qty_match_tag = "🔵 批量入库(无翻单)"
                        qty_match_desc = f"🔵 批量到货入库(实到{int(inbound_delta)}件/无明确翻单)"
                        match_level = 4
                    else:
                        qty_match_tag = "⚪ 零散入库(无翻单)"
                        qty_match_desc = f"⚪ 零散到货入库(实到{int(inbound_delta)}件/无明确翻单)"
                        match_level = 5
                ...
                detail_tag = '/'.join(exclusive_tags)

                if is_in_product_table:
                    shop_status_tag = "✅ 本店在售"
                    shop_type = "on_sale"
                elif is_gift:
                    shop_status_tag = "🎁 本店未录(赠品款)"
                    shop_type = "gift"
                elif is_nba or is_exclusive:
                    tag_suffix = f"[{detail_tag}]" if detail_tag else ""
                    shop_status_tag = f"🟣 非本店在售{tag_suffix}"
                    shop_type = "exclusive"
                elif is_first_order or (code in master_progress_codes or skc in master_progress_skcs or skc in skc_has_any):
                    if is_first_order:
                        shop_status_tag = "🆕 本店未录(首单新品)"
                        shop_type = "new_arrival"
                    elif is_restock_order:
                        shop_status_tag = "⚠️ 本店未录(翻单款待核)"
                        shop_type = "unlisted_restock"
                    else:
                        shop_status_tag = "🆕 本店未录(大货新品)"
                        shop_type = "new_arrival"
                else:
                    shop_status_tag = "❓ 本店未录(异常款)"
                    shop_type = "unknown"
    ...
    def merge_all_restock_entries(restock_dict):
        ...
        for (skc, sz_tuple), item_list in groups.items():
            ...
            # 2. 生产状态融合：优先保留包含在途/生产具体进度的描述
            statuses = [it[2].get('status', '') for it in item_list if it[2].get('status') and str(it[2].get('status')) != 'None']
            detailed_status = ''
            # 最高优先级：在途/待入库/已入库
            for st in statuses:
                if any(kw in str(st) for kw in ['在途', '待入库', '已入库']):
                    detailed_status = str(st)
                    break
            if not detailed_status:
                for st in statuses:
                    if any(kw in str(st) for kw in ['中', '在', '料', '定做', '订做', '改', '裁', '开板', '缺失', '到仓', '待登', '已出清']):
                        detailed_status = str(st)
                        break
            ...
            if master_items:
                merged_d['is_omitted_from_master'] = False
            # 关键：只有当所有条目均已准确入库结清时才标记出清；只要有任一条目为在途/生产中，则绝不标记出清！
            merged_d['is_cleared'] = all(it[2].get('is_cleared', False) for it in item_list)
...
def to_excel(results, neg_skcs, wh_neg_size, wh_colors_txt, unmatched, scores,
    ...
    for d2 in sorted_sheet2:
        ...
        fill_color = 'FFFFFF'
        if '🟢' in remark and '建议开启' in remark:
            fill_color = 'DAEFCE'  # 清新浅绿（本店在售足额到齐）
        elif '🟡' in remark and '限额开启' in remark:
            fill_color = 'FFF2CC'  # 暖黄色高亮（本店在售分批先到）
        elif '🌟' in remark or '待建档上架' in remark or '首单新品' in remark:
            fill_color = 'E2EFDA'  # 柔和浅青绿（新品待建档）
        elif '🟣' in remark or '非本店在售' in remark:
            fill_color = 'E8D7F1'  # 雅致浅紫（专供款/非本店在售）
        elif '⚠️' in remark or '待核实建档' in remark:
            fill_color = 'FCE5CD'  # 柔和浅橙（翻单待核实）
        elif '🎁' in remark or '赠品' in remark:
            fill_color = 'FFF2CC'  # 暖黄赠品色
        elif '🔵' in remark or '运营核实后开启' in remark:
            fill_color = 'D9E1F2'  # 柔和淡紫蓝
        elif '⚪' in remark or '暂缓' in remark or '零星' in remark:
            fill_color = 'F2F2F2'  # 浅灰（零星到货/暂缓操作）
        elif '❓' in remark or '待供应链核对' in remark:
    ...
    draw_legend_box(ws2, 31, [
        ...
        ('E8D7F1', '🟣 非本店在售/分流', '【非本店专供】NBA联名/澳门限定/买断摆摊等专供款式，关注分流，不开启同步'),
        ('FCE5CD', '⚠️ 翻单待核实建档', '【本店未录/翻单款】生产总表有翻单但本店未录，核实是否为其他店铺款或漏建档'),
        ('FFF2CC', '🎁 赠品备货', '【赠品款号】赠品批量到仓，提示客服与仓管备货，无需在店铺上架'),
        ('D9E1F2', '🔵 批量入库待核', '【本店在售】大批量入库(≥30件)但无明确翻单，提示运营与仓库核实后开启'),
        ('F2F2F2', '⚪ 零星散件暂缓', '翻单或首单零星先到(达成率<20%或<15件)，到货量过少，暂缓开启同步或上架'),
